...
class Cell:
    def __init__(self, base_station, cell_init_data):
        ...
        # ---- external PRB allocator (installed by RL xApp) ----
        self.external_prb_allocator = None
        self._last_dl_slice_split = {}

    def set_external_prb_allocator(self, allocator_fn):
        """
        allocator_fn(cell) -> dict {ue_imsi: {"downlink": int, "uplink": int}}
        """
        self.external_prb_allocator = allocator_fn

    def _apply_ue_prb_allocation(self, ue_map):
        # reset
        for ue in self.connected_ue_list.values():
            self.prb_ue_allocation_dict[ue.ue_imsi] = {"downlink": 0, "uplink": 0}
        total_dl = 0
        for ue_imsi, alloc in (ue_map or {}).items():
            if ue_imsi in self.connected_ue_list:
                dl = max(0, int(alloc.get("downlink", 0)))
                self.prb_ue_allocation_dict[ue_imsi]["downlink"] = dl
                total_dl += dl
        # clamp
        if total_dl > self.max_dl_prb and total_dl > 0:
            scale = self.max_dl_prb / total_dl
            for ue in self.connected_ue_list.values():
                ue_imsi = ue.ue_imsi
                dl = int(self.prb_ue_allocation_dict[ue_imsi]["downlink"] * scale)
                self.prb_ue_allocation_dict[ue_imsi]["downlink"] = dl
        # remember per-slice split for state continuity
        split = {}
        for ue in self.connected_ue_list.values():
            st = ue.slice_type or "default"
            split[st] = split.get(st, 0) + self.prb_ue_allocation_dict[ue.ue_imsi]["downlink"]
        self._last_dl_slice_split = split
